...
SHORTCUTS = {"q": "14", "quit": "14", "exit": "14", "b": None, "back": None, "home": None}
...
def paginate(console: Console, items: list, page_size: int = 10) -> None:
    for i in range(0, len(items), page_size):
        ...
        if i + page_size < len(items):
            if Prompt.ask("[dim]More? (y/n)[/dim]", default="y") != "y":
                break
...
class ShadowNetOps:
    ...
    def display_menu(self) -> None:
        self.console.print(f"[{S['title']}]╔═══ Available Tools & Categories ═══╗[/{S['title']}]")
        col1 = []
        col2 = []
        mid = len(MAIN_MENU_ITEMS) // 2 + 1
        for i, (id_, name, desc) in enumerate(MAIN_MENU_ITEMS):
            line = Text.assemble(
                (f"  {id_:>2}", "cyan"),
                (".", "white"),
                (f" {name}", f"bold green" if id_ != "14" else "bold red"),
            )
            if i < mid:
                col1.append(line)
            else:
                col2.append(line)
        cols = Columns([Text("\n").join(col1), Text("\n").join(col2)], equal=True, expand=True)
        self.console.print(Panel(cols, border_style="cyan", box=box.ROUNDED))
        self.console.print(f"[{S['dim']}]Shortcuts: [cyan]q[/cyan]=quit  [cyan]b[/cyan]=back[/{S['dim']}]")
        self.console.print()
    ...
    def check_dependencies(self, category: str) -> bool:
        ...
        if missing_tools:
            ...
            install_choice = Prompt.ask(
                f"[{S['header']}]Would you like to attempt to install the missing tools? (yes/no)[/{S['header']}]",
                default="no"
            )
    ...
    def _confirm_continue(self) -> bool:
        ...
        c = Prompt.ask(f"[{S['header']}]Continue anyway? (yes/no)[/{S['header']}]", default="no")
        return c.lower() in ['yes', 'y', 'true']
    ...
    def osint_tools_menu(self) -> None:
        ...
        if choice == "1":
            ...
            if not BaseTool.validate_input(username):
    ...
    def web_attack_tools_menu(self) -> None:
        ...
        if choice in ("1", "2", "3", "4", "5", "7", "8"):
            ...
            if not URL_PATTERN.match(url):
                self.console.print(f"[{S['error']}]Invalid URL format[/{S['error']}]")
                return
            if choice == "1":
                try:
            ...
            if not DOMAIN_PATTERN.match(domain):
                self.console.print(f"[{S['error']}]Invalid domain format[/{S['error']}]")
                return
            try:
    ...
    def wireless_attack_tools_menu(self) -> None:
        ...
        if choice in ("1", "2", "5", "7", "10", "11", "12"):
            interface = Prompt.ask("Enter wireless interface", default="wlan0")
        if choice == "1":
            try:
            ...
            channel = Prompt.ask("Enter WiFi channel to monitor", default="6")
            try:
            ...
            ssid = Prompt.ask("Enter SSID", default="Free WiFi")
    ...
    def run(self) -> None:
        ...
        while True:
            self.display_menu()
            raw = Prompt.ask(
                f"[{S['accent']}]shadow@netops[/{S['accent']}] [{S['header']}]~$[/{S['header']}]",
            )
            if raw.lower() in ("q", "quit", "exit", "14"):
                ...
                sys.exit(0)
            if raw.lower() in ("b", "back", "home", ""):
                self.console.print()
                continue
            if not raw.isdigit() or int(raw) not in range(1, 15):
                self.console.print(f"[{S['error']}]Invalid choice: {raw}[/{S['error']}]")
                self.console.print()
                continue
            choice = raw
            self.console.print()
    ...
    def malware_analysis_menu(self) -> None:
        ...
        if choice == "1":
            ...
            rule = Prompt.ask("Enter YARA rule file", default="rules.yar")
            target = Prompt.ask("Enter target file/directory")
            ...
            fp = Prompt.ask("Enter PE file path")
            if os.path.isfile(fp):
                try:
            ...
            iface = Prompt.ask("Enter network interface", default="eth0")
            try:
            ...
            fp = Prompt.ask("Enter memory dump path")
            if os.path.isfile(fp):
    ...
    def reverse_engineering_menu(self) -> None:
        ...
        if choice == "b":
            return
        if choice in ("1", "3"):
            fp = Prompt.ask("Enter binary path")
            if os.path.isfile(fp):
                try:
    ...
    def cryptography_tools_menu(self) -> None:
        ...
        if choice == "1":
            ...
            pw = Prompt.ask("Password", default="defaultpassword123")
            try:
            ...
            dec = Prompt.ask("Decode string (or Enter to skip)", default="")
            if dec:
            ...
            host = Prompt.ask("Enter hostname")
            try:
                import socket, ssl
            ...
            kt = Prompt.ask("Key type", choices=["RSA", "AES"], default="RSA")
            if kt == "RSA":
                try:
